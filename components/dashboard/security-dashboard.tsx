  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col rounded-xl border border-border/50 bg-card/30 p-2">
      ...
      {details ? (
        ...
        <div className="min-h-0 flex-1 overflow-y-auto rounded-lg border border-border/50 bg-background/20 px-3 py-2">
          <div className="flex flex-col gap-2.5">
            {sorted.map((item, index) => (
              <div key={item.name} className="grid grid-cols-[4.5rem_1fr_3.5rem] items-center gap-2">
                <span className="truncate text-[11px] text-muted-foreground" title={item.name}>{item.shortName}</span>
                <div className="h-3 overflow-hidden rounded-full bg-muted/60">
                  <div className="h-full rounded-full bg-primary transition-all" style={{ width: `${Math.max((item.value / 100) * 100, 4)}%` }} />
                </div>
                <span className="text-right font-mono text-[11px] font-semibold text-primary">{item.value}分</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <main className="min-h-screen bg-background text-foreground selection:bg-primary/30">
      <div className="relative mx-auto max-w-[1800px] px-4 pb-8 md:px-6">
        ...
        <div className="mt-5 grid items-start gap-2 lg:grid-cols-3 lg:grid-rows-[560px_auto]">
          ...
          <Panel title="员工安全画像" tone="primary" className="flex h-[520px] w-full flex-col" bodyClassName="flex min-h-0 flex-1 flex-col p-2.5">
          ...
          <Panel title="网络安全考评" tone="accent" className="flex h-[520px] w-full flex-col lg:row-start-1 lg:col-start-3" bodyClassName="flex min-h-0 flex-1 flex-col p-2">
            <AssessmentBars data={filteredCapability} />
          </Panel>

          <Panel title="检查发现问题" tone="accent" className="order-4 flex min-h-[320px] flex-col self-start lg:row-start-2 lg:col-start-2 lg:col-span-2" bodyClassName="flex flex-col gap-4 p-3">
