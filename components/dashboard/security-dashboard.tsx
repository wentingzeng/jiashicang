...
function CapabilityCategoryRadar({ data }: { data: SecurityNetworkCapabilityCategory[] }) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border/50 bg-card p-2">
    ...
    <div className="h-[270px] min-h-[270px] w-full min-w-0 shrink-0 overflow-hidden"><ResponsiveContainer width="100%" height={270}><RadarChart data={radarData} outerRadius="55%"><PolarGrid fill="hsl(var(--muted))" fillOpacity={0.2} stroke="hsl(var(--border))" strokeOpacity={0.65} strokeWidth={1} /><PolarAngleAxis dataKey="metric" tick={{ fontSize: 10, fontWeight: 500, fill: "hsl(var(--muted-foreground))" }} tickLine={false} /><PolarRadiusAxis angle={90} domain={[0, 100]} ticks={[0, 25, 50, 75, 100]} tick={{ fontSize: 10, fontWeight: 500, fill: "hsl(var(--muted-foreground))" }} axisLine={false} tickLine={false} />{data.map((item, index) => <Radar key={item.category} name={item.category} dataKey={item.category} stroke={colors[index % colors.length]} fill={colors[index % colors.length]} fillOpacity={0.14} strokeOpacity={0.72} strokeWidth={1.5} dot={{ r: 2, fill: colors[index % colors.length], fillOpacity: 0.8, strokeWidth: 0 }} activeDot={{ r: 3 }} />)}</RadarChart></ResponsiveContainer></div>
