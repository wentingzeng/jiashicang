...
function CapabilityBars({ data, label, selectedInstitution }: { data: { name: string; value: number }[]; label: string; selectedInstitution: string }) {
  ...
  return <div className="h-[245px] rounded-lg border border-border/50 bg-background/20 p-3"><div className="mb-3 flex items-center justify-between text-xs text-muted-foreground"><span className="text-sm font-medium text-foreground/80">{label}</span>{selectedBranch ? <span>点击分行查看各项得分</span> : <button type="button" onClick={() => setShowAll(!showAll)} className="text-sm font-medium text-primary transition-colors hover:underline">{showAll ? "返回突出分行" : "点击查看全部分行"}</button>}</div>{selectedBranch ? <div><div className="mb-2 flex items-center justify-between rounded-lg border border-primary/15 bg-primary/5 px-3 py-2"><span className="text-xs font-semibold text-foreground">{selectedBranch} · 各项得分</span><button type="button" onClick={() => setSelectedBranch(null)} className="rounded-md border border-primary/20 bg-card px-2 py-1 text-[10px] font-medium text-primary transition-colors hover:bg-primary/10">返回总览</button></div><CompactDetailTable className="max-h-72 rounded-xl bg-card/90 text-[11px] shadow-md" headers={["指标", "得分"]} rows={(() => { const item = metrics.find((entry) => entry.name === selectedBranch); return item ? [["全年合��得分", item.value.toFixed(1)], ["压紧压实网络安全责任", item.responsibility.toFixed(1)], ["重要通知和工作部署落实情况及个人信息保护", item.notification.toFixed(1)], ["及时发现及整改网络安全隐患", item.risk.toFixed(1)], ["研发安全", item.research.toFixed(1)], ["总分行一体化安全运行落实情况", item.integrated.toFixed(1)], ["分行网络安全工作亮点及集团贡献情况", item.highlights.toFixed(1)], ["其他扣分项", item.deductions.toFixed(1)]] : []})()} /></div> : <div>{showAll ? <div className="mb-1 text-[10px] text-muted-foreground">全部分行综合能力</div> : <div className="space-y-2"><div><div className="mb-1 text-sm font-bold tracking-wide text-primary">前三名：</div><div className="space-y-1.5">{topMetrics.map(renderBar)}</div></div><div><div className="mb-1 text-sm font-bold tracking-wide text-primary">后三名：</div><div className="space-y-1.5">{bottomMetrics.map(renderBar)}</div></div></div>}<div className="max-h-60 space-y-2 overflow-y-auto pr-1">{(showAll ? visibleMetrics : []).map((item) => <button key={item.name} type="button" onClick={() => setSelectedBranch(item.name)} className="grid w-full grid-cols-[72px_1fr_42px] items-center gap-2 text-left text-[10px] hover:bg-primary/5"><span className="truncate text-muted-foreground">{item.name.replace("分行", "")}</span><span className="h-3 overflow-hidden rounded-full bg-primary/10"><span className="block h-full rounded-full bg-gradient-to-r from-[#4ba8d8] to-[#42bdb7]" style={{ width: `${item.value}%` }} /></span><strong className="text-right font-mono font-normal text-foreground">{item.value.toFixed(1)}</strong></button>)}</div></div>} </div>
...
<div className="flex flex-col gap-2">
  <ChartBox
          ...
          <section className="flex h-full min-h-0 min-w-0 flex-col gap-4">
              <Panel title="检查发现问题" tone="accent" className="flex flex-1 flex-col" bodyClassName="flex min-h-0 flex-1 flex-col gap-3 p-3">
