  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col rounded-xl border border-border/50 bg-card/30 p-2">
      ...
      {selected ? (
        ...
        <div className="min-h-0 flex-1 overflow-y-auto rounded-lg border border-border/50 bg-background/20 px-3 py-2"><div className="flex flex-col gap-2"><div className="grid grid-cols-3 gap-2">{sorted.slice(0, 3).map((item, index) => <button type="button" key={item.name} onClick={() => setSelectedBranch(item.name)} className="flex min-w-0 flex-col gap-2 rounded-lg border border-primary/15 bg-primary/[0.05] p-2.5 text-left transition-colors hover:border-primary/35 hover:bg-primary/10"><div className="flex items-center gap-1.5"><span className="font-mono text-[10px] font-bold text-primary">0{index + 1}</span><span className="truncate text-xs font-medium text-foreground" title={item.name}>{item.shortName}</span></div><span className="font-mono text-sm font-bold text-primary">{item.totalScore.toFixed(2)}分</span></button>)}</div><div className="flex flex-col divide-y divide-border/40 rounded-lg border border-border/50 bg-card/40">{sorted.slice(3).map((item, index) => <button type="button" key={item.name} onClick={() => setSelectedBranch(item.name)} className="grid w-full grid-cols-[2rem_1fr_auto] items-center gap-2 px-2.5 py-2 text-left transition-colors first:rounded-t-lg last:rounded-b-lg hover:bg-primary/5"><span className="font-mono text-[10px] text-muted-foreground">{index + 4}</span><span className="truncate text-xs text-foreground" title={item.name}>{item.shortName}</span><span className="font-mono text-xs font-semibold text-primary">{item.totalScore.toFixed(2)}分</span></button>)}</div></div></div>
      )}
    </div>
  )
}
