...
function CapabilityCategoryRadar({ data }: { data: SecurityNetworkCapabilityCategory[] }) {
  ...
  const colors = ["#1687c4", "#e59b2f", "#18a7a0", "#42a86f", "#7161d8"]
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border/50 bg-card p-2">
    ...
    <div className="relative min-h-0 flex-1 w-full min-w-0 shrink-0 overflow-hidden"><ResponsiveContainer width="100%" height="100%"><RadarChart data={radarData} outerRadius="68%" onClick={(state) => { if (state?.activeLabel) setSelectedMetric(String(state.activeLabel)) }}><PolarGrid fill="none" stroke="hsl(var(--border))" strokeOpacity={0.72} strokeWidth={1} /><PolarAngleAxis dataKey="metric" tick={{ fontSize: 10, fontWeight: 500, fill: "hsl(var(--muted-foreground))" }} tickLine={false} /><PolarRadiusAxis angle={90} domain={[0, 100]} ticks={[0, 25, 50, 75, 100]} tick={{ fontSize: 10, fontWeight: 500, fill: "hsl(var(--muted-foreground))" }} axisLine={false} tickLine={false} /><Tooltip formatter={(value, name) => [Number(value).toFixed(2), name]} labelFormatter={(label) => `指标：${label}`} />{orderedData.map((item, index) => <Radar key={item.category} name={item.category} dataKey={item.category} stroke={colors[index % colors.length]} fill={colors[index % colors.length]} fillOpacity={0.14} strokeOpacity={0.98} strokeWidth={2.2} dot={{ r: 3, fill: colors[index % colors.length], fillOpacity: 1, stroke: "hsl(var(--card))", strokeWidth: 1 }} activeDot={{ r: 5 }} />)}</RadarChart></ResponsiveContainer>{selectedValues && <div className="pointer-events-none absolute bottom-1 left-1/2 -translate-x-1/2 rounded-md border border-border/60 bg-card/95 px-2 py-1 text-[10px] text-foreground shadow-sm">{selectedMetric}：{orderedData.map((item) => `${item.category} ${Number(selectedValues[item.category] ?? 0).toFixed(2)}`).join(" · ")}</div>}</div>
