...
function AssessmentBars({ data }: { data: { name: string; value: number }[] }) {
  ...
  const [details, setDetails] = useState(false)
  return <div className="h-[280px] rounded-xl border border-border/50 bg-background/20 p-2"><div className="mb-3 flex items-center justify-end"><button type="button" onClick={() => setDetails(!details)} className="text-[11px] text-muted-foreground transition-colors hover:text-primary">{details ? "返回总览" : "点击查看详情"}</button></div>{details ? <div className="h-[236px] overflow-y-auto overscroll-contain rounded-xl border border-border/50 bg-card shadow-md"><table className="w-full border-separate border-spacing-0 text-left text-[11px]"><thead className="sticky top-0 z-30 bg-primary text-primary-foreground"><tr><th className="w-16 px-3 py-2 font-semibold">排名</th><th className="px-3 py-2 font-semibold">分行</th><th className="w-24 px-3 py-2 text-right font-semibold">考评得分</th></tr></thead><tbody className="divide-y divide-border/30">{sorted.map((item, index) => <tr key={item.name} className={index % 2 ? "bg-muted/20" : "bg-card/40"}><td className="px-3 py-2 font-mono text-muted-foreground">{String(index + 1).padStart(2, "0")}</td><td className="px-3 py-2 text-foreground">{item.name}</td><td className="px-3 py-2 text-right font-mono text-[11px] font-normal text-primary">{item.value}</td></tr>)}</tbody></table></div> : <div className="h-60 overflow-x-auto overflow-y-hidden overscroll-contain pr-1"><div className="h-full min-w-[620px] px-1"><ResponsiveContainer width="100%" height="100%"><BarChart data={sorted} margin={{ top: 20, right: 12, bottom: 30, left: 8 }} barCategoryGap="4%"><CartesianGrid vertical={false} strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="shortName" interval={0} tick={{ fontSize: 10 }} tickLine={false} axisLine={{ stroke: "hsl(var(--border))" }} /><YAxis domain={[0, 100]} ticks={[0, 20, 40, 60, 80, 100]} tick={{ fontSize: 10 }} tickLine={false} axisLine={{ stroke: "hsl(var(--border))" }} label={{ value: "考评分", angle: -90, position: "insideLeft", style: { fontSize: 10 } }} /><Tooltip formatter={(value) => [`${value}分`, "考评得分"]} labelFormatter={(label) => `${label}`} /><Bar dataKey="value" fill="#e5b45c" radius={[3, 3, 0, 0]} barSize={14}><LabelList dataKey="value" position="top" formatter={(value: unknown) => `${value}分`} style={{ fontSize: 10, fill: "hsl(var(--foreground))" }} /></Bar></BarChart></ResponsiveContainer></div></div>}</div>
}
