  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col rounded-xl border border-border/50 bg-card/30 p-2">
      ...
      {selected ? (
        ...
        <div className="min-h-0 flex-1 overflow-y-auto rounded-lg border border-border/50 bg-background/20 px-3 py-2"><div className="flex flex-col gap-2"><div className="flex min-h-[154px] items-end justify-center"><div className="flex items-end gap-1"><div className="flex w-24 flex-col items-center"><button type="button" onClick={() => setSelectedBranch(podiumItems[0].name)} className="mb-1 flex w-full min-w-0 flex-col items-center rounded-md px-1 py-1 text-center transition-colors hover:bg-primary/5"><span className="line-clamp-2 w-full break-words text-[10px] font-medium leading-tight text-foreground" title={podiumItems[0].name}>{podiumItems[0].shortName}</span><span className="whitespace-nowrap font-mono text-[11px] font-bold text-primary">{podiumItems[0].totalScore.toFixed(2)}分</span></button><button type="button" aria-label={`查看第2名${podiumItems[0].shortName}具体得分`} onClick={() => setSelectedBranch(podiumItems[0].name)} className="flex h-12 w-full cursor-pointer items-start justify-center rounded-t-lg bg-primary/35 pt-1.5 transition-opacity hover:opacity-85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><span className="font-mono text-lg font-extrabold text-primary-foreground">2</span></button></div><div className="flex w-24 flex-col items-center"><button type="button" onClick={() => setSelectedBranch(podiumItems[1].name)} className="mb-1 flex w-full min-w-0 flex-col items-center rounded-md px-1 py-1 text-center transition-colors hover:bg-primary/5"><span className="line-clamp-2 w-full break-words text-[10px] font-medium leading-tight text-foreground" title={podiumItems[1].name}>{podiumItems[1].shortName}</span><span className="whitespace-nowrap font-mono text-[11px] font-bold text-primary">{podiumItems[1].totalScore.toFixed(2)}分</span></button><button type="button" aria-label={`查看第1名${podiumItems[1].shortName}具体得分`} onClick={() => setSelectedBranch(podiumItems[1].name)} className="flex h-20 w-full cursor-pointer items-start justify-center rounded-t-lg bg-primary/70 pt-1.5 transition-opacity hover:opacity-85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><span className="font-mono text-lg font-extrabold text-primary-foreground">1</span></button></div><div className="flex w-24 flex-col items-center"><button type="button" onClick={() => setSelectedBranch(podiumItems[2].name)} className="mb-1 flex w-full min-w-0 flex-col items-center rounded-md px-1 py-1 text-center transition-colors hover:bg-primary/5"><span className="line-clamp-2 w-full break-words text-[10px] font-medium leading-tight text-foreground" title={podiumItems[2].name}>{podiumItems[2].shortName}</span><span className="whitespace-nowrap font-mono text-[11px] font-bold text-primary">{podiumItems[2].totalScore.toFixed(2)}分</span></button><button type="button" aria-label={`查看第3名${podiumItems[2].shortName}具体得分`} onClick={() => setSelectedBranch(podiumItems[2].name)} className="flex h-10 w-full cursor-pointer items-start justify-center rounded-t-lg bg-primary/25 pt-1.5 transition-opacity hover:opacity-85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><span className="font-mono text-lg font-extrabold text-primary-foreground">3</span></button></div></div></div><div className="flex flex-col divide-y divide-border/40 rounded-lg border border-border/50 bg-card/40">{sorted.slice(3).map((item, index) => <button type="button" key={item.name} onClick={() => setSelectedBranch(item.name)} className="grid w-full grid-cols-[2rem_1fr_auto] items-center gap-2 px-2.5 py-2 text-left transition-colors first:rounded-t-lg last:rounded-b-lg hover:bg-primary/5"><span className="font-mono text-[10px] text-muted-foreground">{index + 4}</span><span className="truncate text-xs text-foreground" title={item.name}>{item.shortName}</span><span className="font-mono text-xs font-semibold text-primary">{item.totalScore.toFixed(2)}分</span></button>)}</div></div></div>
      )}
    </div>
  )
}
...
function CapabilityCategoryRadar({ data }: { data: SecurityNetworkCapabilityCategory[] }) {
  ...
  const orderedData = [...data].sort((a, b) => {
    const levelOrder: Record<string, number> = { "一等行": 1, "二等行": 2, "三等行": 3, "四等行": 4, "五等行": 5 }
    const levelOf = (category: string) => levelOrder[category] ?? Number(category.match(/\d+/)?.[0] ?? Number.MAX_SAFE_INTEGER)
    return levelOf(a.category) - levelOf(b.category)
  })
  const radarData = dimensions.map(({ key, label }) => Object.fromEntries([
    ["metric", label],
    ...orderedData.map((item) => [item.category, Number(item[key] ?? 0)]),
  ]))
  const [selectedMetric, setSelectedMetric] = useState<string | null>(null)
  // 按等级固定配色，确保二等行与三等行有明显区分
  const colors = ["#1687c4", "#e59b2f", "#16a6a0", "#7161d8", "#e05a47"]
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border/50 bg-card p-2">
    ...
    <div className="relative min-h-0 flex-1 w-full min-w-0 shrink-0 overflow-hidden"><ResponsiveContainer width="100%" height="100%"><RadarChart data={radarData} outerRadius="68%" onClick={(state) => { if (state?.activeLabel) setSelectedMetric(String(state.activeLabel)) }}><PolarGrid fill="none" stroke="var(--border)" strokeOpacity={0.72} strokeWidth={1} radialLines={true} /><PolarAngleAxis dataKey="metric" tick={{ fontSize: 10, fontWeight: 500, fill: "hsl(var(--muted-foreground))" }} tickLine={false} /><PolarRadiusAxis angle={90} domain={[0, 100]} ticks={[0, 25, 50, 75, 100]} tick={{ fontSize: 10, fontWeight: 500, fill: "hsl(var(--muted-foreground))" }} axisLine={false} tickLine={true} tickSize={4} /><Tooltip content={({ active, label }) => active && <div className="rounded-md border border-border/60 bg-card/95 px-3 py-2 text-xs text-foreground shadow-sm"><div className="mb-1 font-medium">指标：{label}</div>{orderedData.map((item, index) => <div key={item.category} className="flex items-center gap-2 leading-6"><span className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: colors[index % colors.length] }} aria-hidden="true" /><span>{item.category}：</span><span>{Number(radarData.find((row) => row.metric === label)?.[item.category] ?? 0).toFixed(2)}</span></div>)}</div>} />{orderedData.map((item, index) => <Radar key={item.category} name={item.category} dataKey={item.category} stroke={colors[index % colors.length]} fill={colors[index % colors.length]} fillOpacity={0.04} strokeOpacity={1} strokeWidth={2.4} dot={{ r: 3, fill: colors[index % colors.length], fillOpacity: 1, stroke: "hsl(var(--card))", strokeWidth: 1 }} activeDot={{ r: 5 }} />)}</RadarChart></ResponsiveContainer>{selectedValues && <div className="pointer-events-none absolute bottom-1 left-1/2 -translate-x-1/2 rounded-md border border-border/60 bg-card/95 px-2 py-1 text-[10px] text-foreground shadow-sm">{selectedMetric}：{orderedData.map((item) => `${item.category} ${Number(selectedValues[item.category] ?? 0).toFixed(2)}`).join(" · ")}</div>}</div>
    <div className="flex min-h-0 flex-wrap items-center justify-center gap-x-4 gap-y-1 px-2 pb-1 text-[10px] text-muted-foreground" aria-label="雷达图图例">{orderedData.map((item, index) => <span key={item.category} className="inline-flex items-center gap-1 whitespace-nowrap"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: colors[index % colors.length] }} aria-hidden="true" />{item.category}</span>)}</div>
  </div>
}
  ...
  return (
    <main className="min-h-screen bg-background text-foreground selection:bg-primary/30">
      <div className="relative mx-auto max-w-[1800px] px-4 pb-8 md:px-6">
        ...
        <div className="mt-5 grid items-start gap-2 lg:grid-cols-3 lg:grid-rows-[440px_450px]">
          ...
          <Panel title="员工安全画像" tone="primary" className="flex h-full min-h-0 w-full flex-col" bodyClassName="flex min-h-0 flex-1 flex-col p-2.5">
            <div className="flex flex-col gap-3">
              ...
              <div className="grid min-h-0 flex-1 grid-cols-1 gap-2">
                <div className="flex min-w-0 h-[150px] flex-col rounded-lg border border-border/50 bg-background/20 px-2 pb-1 pt-2"><div className="mb-1 flex h-5 shrink-0 items-center justify-between text-xs text-muted-foreground"><span>安全培训覆盖率</span><span className="font-mono text-[10px]">单位：%</span></div><div className="min-h-0 flex-1 w-full overflow-x-auto"><div style={{ minWidth: `${Math.max(training.length * 56, 360)}px`, height: "100%" }}><ResponsiveContainer width="100%" height="100%"><BarChart width={Math.max(training.length * 56, 360)} data={[...training].sort((a, b) => normalizeRatio(b.safetyTrainingCoverage) - normalizeRatio(a.safetyTrainingCoverage)).map((row) => ({ name: row.unitName.replace("分行", ""), value: normalizeRatio(row.safetyTrainingCoverage) }))} margin={{ top: 12, right: 10, bottom: 0, left: -18 }}><CartesianGrid vertical={false} strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="name" tick={{ fontSize: 10, dy: 3 }} tickLine={false} axisLine={false} /><YAxis domain={[0, 100]} ticks={[0, 20, 40, 60, 80, 100]} tick={{ fontSize: 10 }} tickLine={false} axisLine={false} tickFormatter={(value) => `${value}%`} /><Tooltip formatter={(value) => [`${value}%`, "覆盖率"]} /><Bar dataKey="value" fill="#25a8d2" radius={[3, 3, 0, 0]} /></BarChart></ResponsiveContainer></div></div></div>
