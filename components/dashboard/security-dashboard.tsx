...
function CompactDetailTable({ rows, headers }: { rows: string[][]; headers: string[] }) {
  return <div className="max-h-32 overflow-auto rounded-xl border border-border/50 bg-card/70 text-[9px] shadow-sm"><table className="w-full table-fixed text-left"><thead className="sticky top-0 bg-primary/10 text-primary"><tr>{headers.map((header) => <th key={header} className="border-b border-border/50 px-1.5 py-1 font-medium">{header}</th>)}</tr></thead><tbody className="divide-y divide-border/30">{rows.map((row, index) => <tr key={index} className={index % 2 ? "bg-muted/20" : ""}>{row.map((cell, cellIndex) => <td key={cellIndex} className="truncate px-1.5 py-1 text-muted-foreground">{cell}</td>)}</tr>)}</tbody></table></div>
}
