...
function ChartBox({
  ...
  return (
    <div className="flex flex-col rounded-lg border border-border/50 bg-background/20 px-2 pb-2 pt-2" style={{ height: `${height}px` }}>
      ...
      <div className="flex min-h-0 flex-1 items-end gap-1.5 overflow-x-auto pb-0">
        {data.map((item) => {
        ...
        <div className="min-h-0 flex-1 overflow-x-auto overflow-y-hidden overscroll-contain pr-1">
          <div className="h-full min-h-0 min-w-[720px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sorted} margin={{ top: 26, right: 18, left: 8, bottom: 0 }} barCategoryGap="18%">
                <CartesianGrid vertical={false} strokeDasharray="3 3" stroke="hsl(var(--border) / 0.65)" />
                <XAxis dataKey="shortName" interval={0} tick={{ fontSize: 10, dy: 8 }} tickLine={false} axisLine={{ stroke: "hsl(var(--foreground) / 0.65)" }} />
          ...
          <Panel title="员工安全画像" tone="primary" className="flex w-full flex-col" bodyClassName="p-2.5">
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="flex min-w-0 h-[150px] flex-col rounded-lg border border-border/50 bg-background/20 px-2 pb-1 pt-2"><div className="mb-1 flex h-5 shrink-0 items-center justify-between text-xs text-muted-foreground"><span>安全培训覆盖率</span><span className="font-mono text-[10px]">单位：%</span></div><div className="min-h-0 flex-1 w-full"><ResponsiveContainer width="100%" height="100%"><LineChart data={[{ name: "北京", value: 88.4 }, { name: "上海", value: 92.1 }, { name: "广州", value: 86.7 }, { name: "深圳", value: 94.3 }, { name: "杭州", value: 91.6 }, { name: "成都", value: 83.9 }, { name: "南京", value: 89.8 }, { name: "武汉", value: 85.5 }, { name: "西安", value: 81.7 }]} margin={{ top: 12, right: 10, bottom: 0, left: -18 }}><CartesianGrid vertical={false} strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="name" tick={{ fontSize: 10, dy: 8 }} tickLine={false} axisLine={false} /><YAxis domain={[70, 100]} ticks={[70, 80, 90, 100]} tick={{ fontSize: 10 }} tickLine={false} axisLine={false} tickFormatter={(value) => `${value}%`} /><Tooltip formatter={(value) => [`${value}%`, "覆盖率"]} /><Line type="monotone" dataKey="value" stroke="#25a8d2" strokeWidth={2.5} dot={{ r: 3, fill: "#25a8d2", strokeWidth: 1, stroke: "hsl(var(--background))" }} activeDot={{ r: 5 }} /></LineChart></ResponsiveContainer></div></div>
