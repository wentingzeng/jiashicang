          ...
          <div className="flex flex-col gap-2.5">
            {sorted.map((item, index) => (
              <div key={item.name} className="grid grid-cols-[4.5rem_1fr_3.5rem] items-center gap-2">
                ...
                <span className="text-right font-mono text-[11px] font-semibold text-primary">{item.value.toFixed(2)}分</span>
              </div>
            ))}
          </div>
        </div>
...
export function SecurityDashboard() {
  ...
  const mapData = [...new Map(branches.map((row) => [row.province, { name: row.province, value: Number(row.annualTotalScore ?? 0), rankByAllBranches: row.rankByAllBranches, rankByBranchLevel: row.rankByBranchLevel, branchLevel: row.branchLevel }])).values()]
