            ...
            <div className="grid min-w-0 items-stretch gap-3 lg:grid-cols-[minmax(240px,0.34fr)_minmax(0,0.66fr)]">
              ...
              <div className="order-2 col-start-2 row-start-1 row-span-2 grid min-w-0 items-stretch gap-2 grid-cols-2 max-md:grid-cols-1 md:[&>*]:min-w-0">
                  ...
                  <PanelCard className="order-2 h-[248px] overflow-hidden" bodyClassName="p-3" title="科技风险" icon={<ShieldAlert className="size-4" />}>
                    <div className="flex h-full min-h-0 flex-col gap-2"><div className="grid grid-cols-2 gap-2"><div className="flex min-w-0 items-center justify-between gap-1 rounded-lg bg-primary/[0.05] px-2 py-1.5"><span className="whitespace-nowrap text-[11px] text-muted-foreground">发现问题：</span><strong className="shrink-0 font-mono text-sm text-primary">23</strong></div><div className="flex min-w-0 items-center justify-between gap-1 rounded-lg bg-primary/[0.05] px-2 py-1.5"><span className="whitespace-nowrap text-[11px] text-muted-foreground">问题整改率：</span><strong className="shrink-0 font-mono text-sm text-primary">87%</strong></div></div><div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pr-1 [scrollbar-gutter:stable] [scrollbar-width:thin]"><div className="mb-1.5 flex items-center justify-between"><span className="text-xs font-semibold text-foreground">问题分类</span><span className="text-[10px] text-muted-foreground">共 6 类</span></div><div className="grid gap-1.5">{[["网络安全", 8], ["系统运维", 6], ["数据管理", 5], ["项目管理", 4], ["科技合规", 3], ["其他问题", 2]].map(([label, value], index) => <div key={label as string} className="flex items-center justify-between rounded-md border border-border/50 bg-muted/25 px-2.5 py-1.5 text-xs"><span className="flex items-center gap-2"><span className="flex size-5 items-center justify-center rounded-full bg-primary/10 font-mono text-[10px] font-bold text-primary">{index + 1}</span><span className="text-foreground/85">{label}</span></span><strong className="font-mono text-sm text-primary">{value}<span className="ml-1 text-[10px] font-normal text-muted-foreground">项</span></strong></div>)}</div></div></div>
                  </PanelCard>
              </div>
