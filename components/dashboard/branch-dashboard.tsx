            ...
            <div className="grid min-w-0 items-stretch gap-3 lg:grid-cols-[minmax(240px,0.34fr)_minmax(0,0.66fr)]">
              ...
              <div className="order-2 col-start-2 row-start-1 row-span-2 grid min-w-0 items-stretch gap-2 grid-cols-2 max-md:grid-cols-1 md:[&>*]:min-w-0">
                  ...
                  <PanelCard className="h-[248px] overflow-hidden" bodyClassName="p-3" title="项目信息报备" icon={<ClipboardList className="size-4" />}>
                    <div className="flex h-full flex-col justify-center gap-2 rounded-lg border border-primary/10 bg-primary/[0.035] p-2.5">{[["规划报备项目总数", 28], ["在建项目数", 16], ["成果报备项目数", 9]].map(([label, value], index) => <div key={label as string} className="flex min-h-0 flex-1 items-center justify-between rounded-lg border border-border/50 bg-card/70 px-4 py-2.5"><div className="flex min-w-0 items-center gap-3"><span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-primary/10 font-mono text-xs font-bold text-primary">{index + 1}</span><span className="truncate text-sm font-medium text-foreground/85">{label}</span></div><strong className="shrink-0 font-mono text-2xl font-bold leading-none text-primary">{value}<span className="ml-1 text-xs font-normal text-muted-foreground">项</span></strong></div>)}</div>
                  </PanelCard>
