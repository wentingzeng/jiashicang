            ...
            <div className="grid min-w-0 items-stretch gap-3 lg:grid-cols-[minmax(240px,0.34fr)_minmax(0,0.66fr)]">
              ...
              <div className="order-2 col-start-2 row-start-1 row-span-2 grid min-w-0 items-stretch gap-2 grid-cols-2 max-md:grid-cols-1 md:[&>*]:min-w-0">
                  ...
                  <PanelCard className="h-[248px] overflow-hidden" bodyClassName="p-3" title="项目信息报备" icon={<ClipboardList className="size-4" />}>
                    <div className="grid h-full grid-cols-3 divide-x divide-border/60 rounded-lg border border-primary/10 bg-primary/[0.035]">{[["规划报备项目总数", 28], ["在建项目数", 16], ["成果报备项目数", 9]].map(([label, value]) => <div key={label as string} className="flex min-w-0 flex-col items-center justify-center gap-3 px-2 text-center"><strong className="font-mono text-3xl font-bold leading-none text-primary">{value}</strong><span className="text-xs leading-5 text-foreground/80">{label}</span><span className="h-1 w-8 rounded-full bg-primary/30" aria-hidden="true" /></div>)}</div>
                  </PanelCard>
