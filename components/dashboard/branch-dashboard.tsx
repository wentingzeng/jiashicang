            ...
            <div className="grid min-w-0 items-stretch gap-3 lg:grid-cols-[minmax(240px,0.34fr)_minmax(0,0.66fr)]">
              ...
              <div className="order-2 col-start-2 row-start-1 row-span-2 grid min-w-0 items-stretch gap-2 grid-cols-2 max-md:grid-cols-1 md:[&>*]:min-w-0">
                  ...
                  <PanelCard className="h-[248px] overflow-hidden" bodyClassName="p-3" title="项目信息报备" icon={<ClipboardList className="size-4" />}>
                    <div className="flex h-full flex-col justify-center gap-2 rounded-lg border border-primary/10 bg-primary/[0.035] p-2.5">{[["规划报备项目总数", 28], ["在建项目数", 16], ["成果报备项目数", 9]].map(([label, value], index) => <div key={label as string} className="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-lg border border-border/50 bg-card/70 px-3 py-2.5"><div className="flex min-w-0 items-center gap-2"><span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-primary/10 font-mono text-xs font-bold text-primary">{index + 1}</span><span className="min-w-0 whitespace-normal break-keep text-xs font-medium leading-5 text-foreground/85">{label}</span></div><strong className="shrink-0 whitespace-nowrap font-mono text-xl font-bold leading-none text-primary">{value}<span className="ml-1 text-xs font-normal text-muted-foreground">项</span></strong></div>)}</div>
                  </PanelCard>
                  <PanelCard className="h-[248px] overflow-hidden" bodyClassName="p-3" title="科技风险数据" icon={<ShieldAlert className="size-4" />}>
                    <div className="flex h-full min-h-0 flex-col gap-2"><div className="grid grid-cols-2 gap-2"><div className="flex min-w-0 items-center justify-between gap-1 rounded-lg bg-primary/[0.05] px-2 py-1.5"><span className="whitespace-nowrap text-[11px] text-muted-foreground">发现问题：</span><strong className="shrink-0 font-mono text-sm text-primary">23</strong></div><div className="flex min-w-0 items-center justify-between gap-1 rounded-lg bg-primary/[0.05] px-2 py-1.5"><span className="whitespace-nowrap text-[11px] text-muted-foreground">问题整改率：</span><strong className="shrink-0 font-mono text-sm text-primary">87%</strong></div></div><div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pr-1 [scrollbar-width:thin]"><div className="mb-1 text-xs font-semibold text-foreground">问题分类</div>{[["网络安全", 8], ["系统运维", 6], ["数据管理", 5], ["项目管理", 4], ["科技合规", 3], ["其他问题", 2]].map(([label, value], index) => <div key={label as string} className="flex items-center justify-between border-b border-border/40 py-1.5 text-xs last:border-0"><span><span className="mr-1 font-mono text-muted-foreground">{index + 1}</span>{label}</span><strong className="font-mono text-primary">{value}</strong></div>)}</div></div>
                  </PanelCard>
              </div>
