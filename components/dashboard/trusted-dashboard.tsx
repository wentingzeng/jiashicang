...
function SmallMachineBars() {
  ...
  return <div><div className="relative mb-5 ml-8 h-32 border-b border-l border-border pl-0 pt-3"><div className="pointer-events-none absolute inset-y-0 -left-8 flex flex-col justify-between text-[9px] text-muted-foreground"><span>{maxValue}</span><span>180</span><span>0</span></div><div className="flex h-full items-end gap-2 pb-4">{rows.map(([name, rest, done], index) => <button type="button" key={name} onClick={() => setSelected(index)} title={`${name}：尚未下线 ${rest}，已下线 ${done}`} aria-label={`${name}，尚未下线 ${rest}，已下线 ${done}`} className={`relative z-10 flex min-w-0 flex-1 flex-col items-center rounded-t px-0.5 transition ${selected === index ? "bg-primary/10" : "hover:bg-secondary/60"}`}><div className="flex w-full flex-col justify-end" style={{ height: `${((rest + done) / maxValue) * 82}px` }}><div className="w-full rounded-t bg-[#4ba8d8]" style={{ height: `${(rest / (rest + done)) * 100}%` }} /><div className="w-full bg-accent" style={{ height: `${(done / (rest + done)) * 100}%` }} /></div><span className="absolute -bottom-7 left-1/2 max-w-full -translate-x-1/2 truncate text-[9px] text-muted-foreground">{name}</span></button>)}</div><div className="absolute -top-1 right-0 flex gap-2 text-[9px] text-muted-foreground"><span><i className="mr-1 inline-block size-1.5 rounded-sm bg-primary" />未下线</span><span><i className="mr-1 inline-block size-1.5 rounded-sm bg-accent" />已下线</span></div></div><p className="mt-1 text-center text-[10px] text-muted-foreground">{label}：尚未下线 <strong className="font-mono text-foreground">{remaining}</strong>，已下线 <strong className="font-mono text-accent">{offline}</strong></p></div>
}
