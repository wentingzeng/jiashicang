...
function HorizontalBars() {
  ...
  return <div className="flex min-w-0 flex-1 flex-col justify-between gap-4 py-1">{rows.map(([label, actual, plan]) => <div key={label} className="min-w-0"><div className="mb-2 flex items-center justify-between gap-3 text-sm"><span className="truncate font-semibold text-foreground">{label}</span><span className="shrink-0 text-xs text-muted-foreground">实际/计划</span></div><div className="relative flex h-16 min-w-0 flex-col justify-between overflow-hidden rounded-lg bg-secondary/45 p-1"><div className="flex h-6 min-w-0 items-center rounded bg-primary px-3 text-xs font-bold text-primary-foreground" style={{ width: `${Math.max(30, (actual / plan) * 100)}%` }}><span className="truncate">{actual}</span></div><div className="flex h-6 min-w-0 items-center rounded bg-chart-4 px-3 text-xs font-bold text-primary-foreground" style={{ width: "100%" }}><span className="truncate">{plan.toLocaleString()}</span></div></div></div>)}</div>
}
...
function SmallMachineBars() {
  ...
  return <div><div className="relative h-32 border-b border-l border-border pl-7 pt-3"><div className="pointer-events-none absolute inset-y-3 left-1 flex flex-col justify-between text-[9px] text-muted-foreground"><span>{maxValue}</span><span>180</span><span>0</span></div><div className="flex h-full items-end gap-2">{rows.map(([name, rest, done], index) => <button type="button" key={name} onClick={() => setSelected(index)} title={`${name}：尚未下线 ${rest}，已下线 ${done}`} aria-label={`${name}，尚未下线 ${rest}，已下线 ${done}`} className={`relative z-10 flex min-w-0 flex-1 flex-col items-center rounded-t px-0.5 transition ${selected === index ? "bg-primary/10" : "hover:bg-secondary/60"}`}><div className="flex w-full flex-col justify-end" style={{ height: `${((rest + done) / maxValue) * 82}px` }}><div className="w-full rounded-t bg-primary" style={{ height: `${(rest / (rest + done)) * 100}%` }} /><div className="w-full bg-accent" style={{ height: `${(done / (rest + done)) * 100}%` }} /></div><span className="mt-0.5 max-w-full truncate text-[9px] text-muted-foreground">{name}</span></button>)}</div><div className="absolute -top-1 right-0 flex gap-2 text-[9px] text-muted-foreground"><span><i className="mr-1 inline-block size-1.5 rounded-sm bg-primary" />未下线</span><span><i className="mr-1 inline-block size-1.5 rounded-sm bg-accent" />已下线</span></div></div><p className="mt-1 text-center text-[10px] text-muted-foreground">{label}：尚未下线 <strong className="font-mono text-foreground">{remaining}</strong>，已下线 <strong className="font-mono text-accent">{offline}</strong></p></div>
}
