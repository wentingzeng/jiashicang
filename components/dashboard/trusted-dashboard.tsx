...
export function TrustedDashboard() {
  ...
  return <div className="min-h-screen bg-background text-foreground"><div className="relative mx-auto max-w-[1800px] px-4 pb-8 md:px-6">
    ...
    <div className="mb-6 mt-4 flex flex-wrap items-center justify-end gap-2 rounded-xl border border-border/60 bg-card/80 p-3 shadow-[0_12px_28px_oklch(0.35_0.06_240/10%)]"><label className="flex min-w-44 items-center gap-3 rounded-md border border-border/60 bg-background/35 px-3 py-2 text-sm"><span className="text-muted-foreground">统计年度</span><select value={year} onChange={(e) => setYear(e.target.value)} className="w-full bg-transparent font-semibold text-foreground outline-none"><option>2026</option><option>2025</option></select><ChevronDown className="size-4" /></label></div>
    <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(280px,0.92fr)_minmax(620px,2.2fr)_minmax(300px,0.92fr)]">
