...
export function TrustedDashboard() {
  return <div className="min-h-screen bg-background text-foreground"><div className="relative mx-auto max-w-[1800px] px-4 pb-8 md:px-6">
    ...
    <div className="grid min-w-0 items-stretch gap-5 lg:grid-cols-12">
      <div className="order-2 col-span-12 flex h-full min-h-0 flex-col gap-5 lg:col-span-6"><Panel className="flex flex-1 flex-col" title="软件类产品许可部署进度"><p className="mb-3 text-sm font-semibold text-foreground">总体进度</p><MiniBars rows={rolloutRows} /></Panel><Panel className="flex flex-1 flex-col" title="国产改造和替代进度"><p className="mb-4 text-sm font-semibold text-foreground">总体进度</p><div className="grid grid-cols-2 gap-3">{[["密评工作", 100, 30], ["国密改造", 80, 25]].map(([label, total, done]) => <div key={label} className="rounded-xl bg-secondary/60 p-4"><p className="text-sm font-bold text-foreground">{label}</p><p className="mt-2 font-mono text-3xl font-black text-primary">{total}</p><p className="text-xs text-muted-foreground">总数</p><div className="mt-3 text-xs text-muted-foreground">已改造数 <strong className="font-mono text-accent">{done}</strong></div></div>)}</div></Panel></div>
      <section className="order-1 col-span-12 min-w-0 rounded-xl border-4 border-primary/80 lg:col-span-12 bg-card p-4 shadow-[0_10px_35px_oklch(0.35_0.06_240/10%)]"><div className="border-b border-border pb-3 text-center"><h2 className="text-xl font-black text-primary">核心指标</h2><p className="mt-1 text-xs text-muted-foreground">信创改造关键进展</p></div><div className="grid gap-4 p-2"><div className="grid gap-4 lg:grid-cols-3"><div><h3 className="mb-3 text-sm font-bold text-foreground">一般系统信创进度</h3><Donut value={65.81} label="已替代" total="412" /></div><div><h3 className="mb-3 text-sm font-bold text-foreground">2026年任务进度</h3><Donut value={24.68} label="已完成" total="58" color={palette.violet} /></div><div><h3 className="mb-3 text-sm font-bold text-foreground">核心系统信创进度</h3><HorizontalBars /></div></div><div className="grid gap-4 lg:grid-cols-2"><div><h3 className="mb-3 text-sm font-bold text-foreground">麒麟界面推广进度</h3><Donut value={19.56} label="已推广" total="16.68K" color={palette.violet} /><h3 className="mb-3 mt-4 text-sm font-bold text-foreground">2026年任务进度</h3><Donut value={30.93} label="年度已推广" total="16.68K" color={palette.violet} /></div><div><h3 className="mb-3 text-sm font-bold text-foreground">A4单色打印机存量替代进度</h3><Donut value={38.15} label="推广数" total="4,572" color={palette.violet} /><h3 className="mb-3 mt-4 text-sm font-bold text-foreground">小型机下线进展</h3><SmallMachineBars /></div></div></div></section>
      <div className="order-3 col-span-12 flex h-full min-h-0 flex-col gap-5 lg:col-span-6"><DetailTable className="flex flex-1 flex-col" title="总行系统改造进展明细" rows={departmentRows} /><DetailTable className="flex flex-1 flex-col" title="分行系统改造进展明细" rows={branchRows} branch /></div>
    </div>
  </div></div>
}
