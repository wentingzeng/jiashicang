...
function HorizontalBars() {
  ...
  return <div className="flex min-w-0 flex-col gap-5">{rows.map(([label, actual, plan]) => <div key={label} className="min-w-0"><div className="mb-2 flex items-center justify-between gap-3 text-sm"><span className="truncate font-semibold text-foreground">{label}</span><span className="shrink-0 text-xs text-muted-foreground">实际 / 计划</span></div><div className="relative flex h-[4.5rem] min-w-0 flex-col justify-between overflow-hidden rounded-md bg-secondary/45 p-1"><div className="flex h-7 min-w-0 items-center rounded bg-primary px-3 text-xs font-bold text-primary-foreground" style={{ width: `${Math.max(22, (actual / plan) * 100)}%` }}><span className="truncate">实际天数 {actual}</span></div><div className="flex h-7 min-w-0 items-center rounded bg-chart-4 px-3 text-xs font-bold text-primary-foreground" style={{ width: "100%" }}><span className="truncate">计划天数 {plan.toLocaleString()}</span></div></div></div>)}</div>
}

function MiniBars({ rows }: { rows: Array<[string, number, number]> }) {
  const maxValue = Math.max(...rows.map(([, value]) => value), 1)
  return <div className="flex h-40 items-end gap-2 border-b border-l border-border px-3 pb-0 pt-4">{rows.map(([label, value, percent]) => <div key={label} className="flex min-w-0 flex-1 flex-col items-center justify-end gap-1"><span className="font-mono text-[10px] font-bold text-primary">{percent}%</span><div className="w-full rounded-t-md bg-primary" style={{ height: `${Math.max(16, (value / maxValue) * 112)}px` }} /><span className="max-w-full truncate text-[10px] text-muted-foreground">{label}</span></div>)}</div>
}

function SmallMachineBars() {
  const rows = [["南京", 328, 86], ["苏州", 286, 74], ["北京", 240, 62], ["上海", 178, 48], ["其他", 120, 36]]
  const maxValue = 360
  return <div className="relative h-44 border-b border-l border-border pl-7 pt-4"><div className="absolute inset-y-4 left-0 flex flex-col justify-between text-[10px] text-muted-foreground"><span>360</span><span>240</span><span>120</span><span>0</span></div><div className="flex h-full items-end gap-3">{rows.map(([label, remaining, offline]) => <div key={label} className="flex min-w-0 flex-1 items-end gap-0.5"><div className="flex min-w-0 flex-1 flex-col items-center"><div className="w-full rounded-t bg-primary" style={{ height: `${(remaining / maxValue) * 112}px` }} /><span className="mt-1 max-w-full truncate text-[10px] text-muted-foreground">{label}</span></div><div className="flex min-w-0 flex-1 flex-col items-center"><div className="w-full rounded-t bg-accent" style={{ height: `${(offline / maxValue) * 112}px` }} /><span className="mt-1 text-[10px] text-muted-foreground">已下线</span></div></div>)}</div><div className="absolute bottom-0 right-0 flex gap-3 text-[10px] text-muted-foreground"><span><i className="mr-1 inline-block size-2 rounded-sm bg-primary" />尚未下线数</span><span><i className="mr-1 inline-block size-2 rounded-sm bg-accent" />已下线数</span></div></div>
}
